...
for (T in -20:30)
{
  hourly <- sapply(hourly_models, function(x)
    x(rep(T, 1000)))[1000, ]
  ...
  daily <- sapply(daily_models, function(x)
    x(temp_frame))[1000, ]
  
  if (T == -20)
    sensitivity <- c(T = T, daily, hourly)
  else
...
KA_temps_JD <-
  make_JDay(read_tab("data/TMaxTMin1958-2019_patched.csv"))
temps <- stack_hourly_temps(KA_temps_JD[which(KA_temps_JD$JDay > 305 |
                                                KA_temps_JD$JDay < 31), ],
                            latitude = 50.6)
hh <- hist(temps$hourtemps$Temp,
           breaks = c(-30:30),
           plot = FALSE)
hh_df <- data.frame(
  T = hh$mids,
...
temps <- stack_hourly_temps(Beijing_temps_JD[which(Beijing_temps_JD$JDay >
                                                     305 | Beijing_temps_JD$JDay < 31), ]
                            , latitude = 39.9)
hh <- hist(temps$hourtemps$Temp,
           breaks = c(-30:30),
           plot = FALSE)
...
temps <- stack_hourly_temps(Davis_temps_JD[which(Davis_temps_JD$JDay > 305 |
                                                   Davis_temps_JD$JDay < 31), ],
                            latitude = 38.5)
hh <- hist(temps$hourtemps$Temp,
           breaks = c(-30:40),
           plot = FALSE)
...
locations <-
  ...
  ylab("Location") +
  xlab("Temperature (between November and January, °C)") +
  xlim(c(-30, 40)) +
  theme_bw(base_size = 15) +
  labs(size = "Relative \nfrequency")
